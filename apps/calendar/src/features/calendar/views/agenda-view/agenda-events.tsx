  ...
  return (
    <Command className="py-4 h-[80vh] bg-transparent">
      <div className="mb-4 mx-4">
        <CommandInput placeholder={m.calendar_agenda_search_placeholder()} />
      </div>
      <CommandList className="max-h-max px-3 border-t">
          ...
          >
            {groupedEvents!.map((event) => (
              <CommandItem
                key={event.id}
                className={cn(
                  "mb-2 p-4 border rounded-md data-[selected=true]:bg-accent transition-all hover:cursor-pointer",
                  {
                    ...
                    <div className="w-40 flex justify-center items-center gap-1">
                      {agendaModeGroupBy === "date" ? (
                        ...
                        <>
                          ...
                          <span className="text-sm">{m.calendar_agenda_at()}</span>
                          <p className="text-sm">
                            {formatTime(event.startDate, use24HourFormat)}
                          </p>
                        </>
        ...
        <CommandEmpty>{m.calendar_agenda_no_results()}</CommandEmpty>
      </CommandList>
    </Command>
  );
};
